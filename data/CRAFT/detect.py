...
def detect_single_image(img, model, device, cfg):
	...
	boxes, M = restore_boxes(region, affinity, cfg.region_thresh, cfg.affinity_thresh, cfg.remove_thresh, (ratio_h, ratio_w))
...
if __name__ == '__main__':
	img_files = [img_file for img_file in sorted(os.listdir('../aihub_images'))]
	...
	for img_path in tqdm(img_files):
		img = Image.open(os.path.join('../aihub_images', img_path))
		img_npy = img_path.replace('jpg', 'npy')
		img_txt = img_path.replace('jpg', 'txt')
		boxes, region, affinity, M = detect_single_image(img, model, device, cfg.test)
		seq = []
		for box in boxes:
			x_min = min(box[0], box[2], box[4], box[6])
			x_max = max(box[0], box[2], box[4], box[6])
