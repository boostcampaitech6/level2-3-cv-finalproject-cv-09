...
font_layout = ImageFont.truetype("../../logo_generation/textdiffuser-2/arial.ttf", 32)
...
def text_to_image(
    user_dir, keywords,
    prompt,
    positive_prompt=', digital art, very detailed, fantasy, high definition,\
        cinematic light, dnd, trending on artstation',
    sampling_step=20,
    guidance=7.5,
    batch=4,
    natural=False,
    ...
    with torch.no_grad():
        ...
        template = f'Given a prompt that will be used to generate an image, \
            plan the layout of visual text for the image. The size of the \
            image is 128x128. Therefore, all properties of the positions \
            should not exceed 128, including the coordinates of top, left, \
            right, and bottom. In addition, we also provide all keywords \
            at random order for reference. You dont need to specify the \
            details of font styles. At each line, the format should be \
            keyword left, top, right, bottom. So let us begin. \
            Prompt: {prompt}. Keywords: {str(keywords)}'
