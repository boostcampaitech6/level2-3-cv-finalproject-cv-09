...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        try: #### note that few cases do not contain valid captions
            caption = open(f'{self.dataset_path}/{path}/{image_name}/caption.txt').readlines()[0]
        except:
            caption = 'null'
        ...
        ocrs = open(f'{self.dataset_path}/{path}/{image_name}/ocr.txt').readlines()
        ...
        examples = {}
        examples["images"] = [self.image_transforms(image).sub_(0.5).div_(0.5) for image in images]
...
def main():
    ...
    def preprocess_train(root_folder):
        images = []
        prompts_train = []
        prompts_cond = []
        prompts_nocond = []
        examples = sorted(os.listdir(root_folder))
        for image_paths in examples:
            image_name = os.listdir(f'{args.dataset_path}/{image_paths}')
            image_name = image_name[0]
            image_paths = image_paths.split('/')[-1]
            #### get image
            image_path = f'{args.dataset_path}/{image_paths}/{image_name}/image.jpg'
            image = Image.open(image_path).convert("RGB")
            images.append(image)

            #### get caption
            try: #### note that few cases do not contain valid captions
                caption = open(f'{args.dataset_path}/{image_paths}/{image_name}/caption.txt').readlines()[0]
            except:
                caption = 'null'
                print('erorr of caption')
            #### get ocr
            #### since the original ocr annotations are word-level, we need to merge some boxes to construct line-level ocr
            ocrs = open(f'{args.dataset_path}/{image_paths}/{image_name}/ocr.txt').readlines()
    ...
    with accelerator.main_process_first():
        ...
        root_folder = '/data/ephemeral/home/dataset/final'
                    ...
                    if (step + 0) % args.vis_interval == 0:           
                        ...
                        for t in tqdm(scheduler.timesteps):
                            with torch.no_grad():  # classifier free guidance
                                ...
                                print(input.shape, t, encoder_hidden_states_cond.shape)
